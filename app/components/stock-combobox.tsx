...
export function StockCombobox({
  ...
  /** Symbols already picked elsewhere and hidden from this dropdown. */
  exclude?: string[];
  ...
  const listId = useId();
  const excludeKey = exclude.map((symbol) => symbol.toUpperCase()).sort().join(",");
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        const nextSuggestions = (data.suggestions ?? []).filter((suggestion) => !excluded.has(suggestion.symbol.toUpperCase()));
        setSuggestions(nextSuggestions);
        setTotal(data.total ?? nextSuggestions.length);
        setActive(0);
      } catch {
  ...
  return (
    <div ref={wrapper} className={`relative min-w-0 ${className}`}>
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <input
            ...
            role="combobox"
            aria-label={placeholder ?? "Search any listed stock"}
            aria-expanded={open}
