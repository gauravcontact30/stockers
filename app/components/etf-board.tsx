    ...
    <li className="rounded-2xl border border-slate-200 bg-slate-50 p-4 transition hover:border-slate-300 dark:border-slate-800 dark:bg-slate-950/60 dark:hover:border-slate-700">
      ...
      <dl className="mt-3 grid grid-cols-2 gap-x-3 gap-y-2 border-t border-slate-200 pt-3 text-[11px] sm:grid-cols-4 dark:border-slate-800">
        ...
        <div>
          ...
          <dd className={`mt-0.5 font-semibold tabular-nums ${toneFor(etf.changePercent365d)}`}>
            {formatSignedPercent(etf.changePercent365d)}
          </dd>
        </div>
      </dl>
...
export function EtfBoard() {
  ...
  return (
    <MarketSection
      ...
      title="ETFs by asset class, ranked by money traded"
      blurb="Every ETF listed on NSE, grouped by what it actually holds — gold, silver, the Nifty 50, overseas indices and more — and ranked within each group by the rupee value traded today."
      aside={
    ...
    >
      ...
      {!loading && groups.length > 0 && selected && (
        <>
          ...
          <ul className="mt-4 grid gap-3 lg:grid-cols-2">
            {selected.etfs.map((etf, index) => (
              <EtfCard key={etf.symbol} etf={etf} rank={index + 1} />
            ))}
          </ul>
        </>
      )}
      ...
      <SectionFootnote>
        Live ETF data from NSE, ranked by rupee turnover. Net buying versus selling is not published for ETFs, so
        &quot;most bought&quot; here means most money traded · not investment advice.
      </SectionFootnote>
    </MarketSection>
  );
}
